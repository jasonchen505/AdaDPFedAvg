...
class BaseTrainer(object):
    def __init__(self, options, dataset, model=None, optimizer=None, name='', worker=None):
        ...
        print('>>> Initialize {} clients in total'.format(self.clients_num))
        ...
        # Calculate the basic privacy loss for each selected cases
    ...
    def setup_clients(self, dataset):
        ...
        for user, group in zip(users, groups):
            # simply clip too long name
            # is instance: judge whether is instance (apparently XD)
            if isinstance(user, str) and len(user) >= 5:
                user_id = int(user[-5:])
            else:
                user_id = int(user)
            self.data_num.append(len(train_data[user]))
            self.all_train_data_num += self.data_num[i]
            c = Client(user_id, group, train_data[user], test_data[user], self.batch_size, self.worker, self.data_num[i])
            i += 1
